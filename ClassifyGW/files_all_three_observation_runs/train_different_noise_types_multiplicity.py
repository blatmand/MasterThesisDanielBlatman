# -*- coding: utf-8 -*-
"""
Created on Sun Feb 20 19:47:34 2022
...
The plots are saved as png files. This script is about the parameter
"train_multiplicity", which empirically shows that a higher train_multiplicity
results in a better classification accuracy to a certain degree. 
When train_multiplicity is 7 a plot of the balanced accuracy as afucntion of 
the event masses is drawn and saved.      
...
for train_multiplicity in train_multiplicities:
    ...
    if(train_multiplicity == 7):
        ...
        plt.tight_layout()
        plt.savefig("masses_plot_C_"+str(C)+"gamma_"+str(gamma)+"_train_multiplicity_"+str(train_multiplicity))
        plt.close()
    ...
    plt.ylim(0,1)
    plt.title("Average balanced accuracy "+ str(attribute)+" features"+ " C = "+str(C)+" gamma = "+str(gamma)+" train multiplicity = "+str(train_multiplicity))
    plt.savefig("Average_accuracy "+attribute+"C_"+str(C)+"gamma_"+str(gamma)+"_train_multiplicity_"+str(train_multiplicity))  
    plt.close()